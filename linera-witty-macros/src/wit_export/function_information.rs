...
impl<'input> FunctionInformation<'input> {
    ...
    fn is_reentrant(signature: &Signature) -> bool {
        ...
        let FnArg::Typed(PatType { ty: first_parameter_type, .. }) = first_parameter else {
            abort!(first_parameter, "`self` parameters aren't supported by Witty");
        };

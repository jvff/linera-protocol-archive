    ...
    ($application:ty) => {
        ...
        extern "C" fn initialize(
            ...
            let result_address = GuestPointer::from(unsafe { CONTRACT_RETURN_AREA.as_mut_ptr() })
                .after_padding_for::<Result<$crate::ExecutionOutcome<Vec<u8>>, String>>(
            );

            result
                .map($crate::ExecutionOutcome::into_raw)
        ...
        extern "C" fn handle_application_call(
            ...
            let result_address = GuestPointer::from(unsafe { CONTRACT_RETURN_AREA.as_mut_ptr() })
                ...
                $crate::ApplicationCallOutcome<Vec<u8>, Vec<u8>, Vec<u8>>,
                String,
            >>();

            result
                .map($crate::ApplicationCallOutcome::into_raw)
                .store(&mut memory, result_address)
        ...
        extern "C" fn handle_session_call(
            ...
            let result_address = GuestPointer::from(unsafe { CONTRACT_RETURN_AREA.as_mut_ptr() })
                .after_padding_for::<Result<($crate::RawSessionCallOutcome, Vec<u8>), String>>(
            );

            result
                .map($crate::SessionCallOutcome::into_raw)
                .store(&mut memory, result_address)

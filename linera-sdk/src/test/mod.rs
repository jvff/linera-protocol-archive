...
use crate::{ApplicationId, BlockHeight, BytecodeId, ChainId, EffectId};
use cargo_toml::Manifest;
use dashmap::DashMap;
use linera_base::{committee::Committee, crypto as base_crypto, data_types as base};
...
impl ActiveChain {
    ...
    pub async fn add_block(&self, block_builder: impl FnOnce(&mut Block)) {
        ...
        let mut block = Block::new(
            self.description.into(),
            base::Owner(self.key_pair.public()),
            tip.as_ref(),
        ...
        self.validator
            ...
            .lock()
            .await
            .fully_handle_certificate(dbg!(certificate.clone()), vec![])
            .await
    ...
    pub async fn subscribe_to_published_bytecodes_from(&mut self, publisher: &mut ActiveChain) {
        dbg!("Subscribe");
        self.add_block(|block| {
...
impl Block {
    ...
    async fn sign(mut self, validator: &TestValidator) -> chain::Certificate {
        ...
        let value = chain::HashedValue::new_confirmed(dbg!(self.block), effects, state_hash);

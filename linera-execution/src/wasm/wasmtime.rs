...
use crate::{
    ...
    Bytecode, CalleeContext, ContractRuntime, ExecutionError, FinalizeContext, MessageContext,
    ...
    Runtime: ContractRuntime + Send + Sync + 'static,
{
    ...
    fn handle_application_call(
        ...
        self.configure_initial_fuel()?;
        let result = ContractEntrypoints::new(&mut self.instance).handle_application_call(argument);
        self.persist_remaining_fuel()?;
        result
            .map_err(WasmExecutionError::from)?
            .map_err(ExecutionError::UserError)

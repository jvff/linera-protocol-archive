...
impl<'runtime> ApplicationRuntimeContext for Contract<'runtime> {
    ...
    fn finalize(context: &mut WasmRuntimeContext<Self>) {
        ...
        let initial_fuel =
            runtime.sync_request(|response| ContractRequest::RemainingFuel { response });
        let remaining_fuel = initial_fuel - context.store.fuel_consumed().unwrap_or(0);

        runtime.sync_request(|response| ContractRequest::SetRemainingFuel {
            remaining_fuel,
            response,
        });
    }
...
impl<'runtime> common::Contract for Contract<'runtime> {
    ...
    fn configure_fuel(context: &mut WasmRuntimeContext<Self>) {
        ...
        context
            .store
            .add_fuel(runtime.sync_request(|response| ContractRequest::RemainingFuel { response }))

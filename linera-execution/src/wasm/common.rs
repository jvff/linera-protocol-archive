...
pub trait Service: ApplicationRuntimeContext {
    ...
    type QueryApplication: GuestFutureInterface<Self, Output = Vec<u8>, Parameters = (QueryContext, Vec<u8>)>
        + Send
        + Unpin;
    ...
    A: Service + Send + Unpin + 'static,
{
    ...
    /// ```
    pub fn query_application(self, context: &QueryContext, argument: &[u8]) -> PollSender<Vec<u8>> {
        GuestFutureActor::<A::QueryApplication, A>::spawn((*context, argument.to_owned()), self)
    }
}

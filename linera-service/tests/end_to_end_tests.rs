...
use self::common::{NodeService, INTEGRATION_TEST_GUARD};
...
async fn test_wasm_end_to_end_counter(config: impl LineraNetConfig) -> Result<()> {
    ...
    let (contract, service) =
        tokio::time::timeout(Duration::ZERO, client.build_example("counter")).await??;
    ...
    let application_id = client
        ...
        .await?;
    let mut node_service = NodeService::from(client.run_node_service(None).await?);
...
async fn test_wasm_end_to_end_social_user_pub_sub(config: impl LineraNetConfig) -> Result<()> {
    ...
    let mut node_service1 = NodeService::from(client1.run_node_service(8080).await?);
    let mut node_service2 = NodeService::from(client2.run_node_service(8081).await?);
...
async fn test_wasm_end_to_end_same_wallet_fungible(
    ...
    let mut node_service = NodeService::from(client1.run_node_service(8080).await?);
...
async fn test_wasm_end_to_end_matching_engine(config: impl LineraNetConfig) -> Result<()> {
    ...
    let mut node_service_admin = NodeService::from(client_admin.run_node_service(8080).await?);
    let mut node_service_a = NodeService::from(client_a.run_node_service(8081).await?);
    let mut node_service_b = NodeService::from(client_b.run_node_service(8082).await?);
...
async fn test_wasm_end_to_end_amm(config: impl LineraNetConfig) -> Result<()> {
    ...
    let chain1 = client_admin.open_and_assign(&client1, Amount::ONE).await?;
    ...
    let mut node_service_admin = NodeService::from(client_admin.run_node_service(8080).await?);
    let mut node_service0 = NodeService::from(client0.run_node_service(8081).await?);
    let mut node_service1 = NodeService::from(client1.run_node_service(8082).await?);
...
async fn test_end_to_end_reconfiguration(config: LocalNetConfig) -> Result<()> {
    ...
    let node_service_2 = match network {
        Network::Grpc => Some(NodeService::from(client_2.run_node_service(8081).await?)),
...
async fn test_open_chain_node_service(config: impl LineraNetConfig) -> Result<()> {
    ...
    let node_service = NodeService::from(client.run_node_service(8080).await?);
...
async fn test_project_publish(database: Database, network: Network) -> Result<()> {
    ...
    let node_service = NodeService::from(client.run_node_service(None).await?);

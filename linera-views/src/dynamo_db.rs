...
impl<E> DynamoDbContext<E> {
    ...
    /// context prefix. The Vec<u8> expression is obtaind from self.extend_prefix
    fn build_key(&self, key: Vec<u8>) -> HashMap<String, AttributeValue> {
        [
            (
        ...
        Item: DeserializeOwned,
    {
        let response = self
            ...
            .get_item()
            .table_name(self.table.as_ref())
            .set_key(Some(self.build_key(self.extend_prefix(key))))
            .send()
            .await?;

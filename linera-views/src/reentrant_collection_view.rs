    ...
    W: View<C> + Send + Sync + 'static,
{
    ...
    pub async fn try_load_entries(
        ...
        let storage_checks = FuturesUnordered::new();
        ...
        for (position, short_key) in short_keys.into_iter().enumerate() {
            if let Some(update) = updates.get(&short_key) {
                ...
                let check = tokio::spawn(async move {
                    let has_key = context.contains_key(&key_index).await?;
                    Ok(has_key.then_some((position, context, short_key)))
                })
                .map(|result_in_result| match result_in_result {
                    Ok(Ok(Some((position, context, short_key)))) => {
                        let subview_key = context.base_tag_index(KeyTag::Subview as u8, &short_key);
                        let subview_context = self.context.clone_with_base_key(subview_key);
                        Ok(Some((position, short_key.to_owned(), subview_context)))
                    }
                    Ok(Ok(None)) => Ok(None),
                    Ok(Err(view_error)) => Err(view_error),
                    Err(join_error) => Err(ViewError::TokioJoinError(join_error)),
                });

                storage_checks.push(check);
            }
        }

        let entries_to_load = storage_checks
            .try_filter_map(|maybe_metadata| async move { Ok(maybe_metadata) })
            .try_collect::<Vec<_>>()
            .await?;
        if !entries_to_load.is_empty() {
            let num_init_keys = W::NUM_INIT_KEYS;
            let mut keys_to_load = Vec::with_capacity(entries_to_load.len() * num_init_keys);
            for (_, _, context) in entries_to_load.iter() {

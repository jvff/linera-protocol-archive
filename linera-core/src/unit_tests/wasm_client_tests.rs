    ...
    ViewError: From<<B::Storage as Storage>::ContextError>,
{
    ...
    let mut publisher = builder
        .add_initial_chain(ChainDescription::Root(0), Amount::from_tokens(3000))
        .await?;
    let mut creator = builder
        .add_initial_chain(ChainDescription::Root(1), Amount::from_tokens(1000))
        .await?;
    ...
    assert!(balance_after_messaging < Amount::from_tokens(1000));
    ...
    ViewError: From<<B::Storage as Storage>::ContextError>,
{
    ...
    let mut creator = builder
        .add_initial_chain(ChainDescription::Root(1), Amount::from_tokens(1))
        .await?;
    // Will receive the message.
    let mut receiver = builder
        .add_initial_chain(ChainDescription::Root(2), Amount::from_tokens(1))
        .await?;
    ...
    ViewError: From<<B::Storage as Storage>::ContextError>,
{
    let mut builder = TestBuilder::new(storage_builder, 4, 1)
        ...
        .with_policy(ResourceControlPolicy::all_categories());
    let mut sender = builder
        .add_initial_chain(ChainDescription::Root(0), Amount::from_tokens(1000))
        .await?;
    let mut receiver = builder
        .add_initial_chain(ChainDescription::Root(1), Amount::from_tokens(1000))
        .await?;
